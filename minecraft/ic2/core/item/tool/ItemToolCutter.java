...
public class ItemToolCutter extends ItemIC2 {
   ...
   public ItemToolCutter(int id, int sprite) {
      super(id, sprite);
   ...
   public static void cutInsulationFrom(ItemStack itemstack, World world, int i, int j, int k) {
      ...
      if(te instanceof TileEntityCable) {
         ...
         if(cable.tryRemoveInsulation()) {
            if(IC2.platform.isSimulating()) {
               double d = (double)world.rand.nextFloat() * 0.7D + 0.15D;
               double d1 = (double)world.rand.nextFloat() * 0.7D + 0.15D;
               double d2 = (double)world.rand.nextFloat() * 0.7D + 0.15D;
